...
# Doesn't work. Parameters can't be type = rep
```{r}
cts <- read.table("Barrett_et_al_2022_CeNGEN_bulk_RNAseq_data.csv", header=TRUE, sep=",", row.names=1)
#make a condition table.csv
condition = read.table("experimental_conditions_reps.csv", header = TRUE, sep=",", row.names=1)
...
sizeFactors(dds)
normalized_counts <- counts(dds, normalized=TRUE)
normalized_countsdata <- write.table(normalized_counts, file="data/normalized_counts.txt", sep="\t", quote=F, col.names = NA) # Use for UMAP later
```
...
```{r}
# Need to find gene names for log2foldchange values > 3
...
# Run this before the plot chunk to change whether we graph reps (assay(vsd) %>% t()) or genes:
# This chunk has been obsoleted as of 8-3-22 when we chose to not perform assay() before normalized_counts
```{r}
...
# umap_results <- umap(normalized_counts, config = custom.settings, labels=as.factor(row.names(normalized_counts)), controlscale=TRUE, scale=3)
```
...
# Find a way to combine replicates here:
```{r}
