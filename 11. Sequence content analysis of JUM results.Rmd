---
title: "Sequence content analysis of JUM results (for HOMER)"
...
# Read JUM excel files created in 7. Cell type vs cell type heatmap script for JUM analysis.Rmd:
```{r}
setwd("E:/Zach Wolfe's JUM analysis/JUM_results")

for (i in singletypes){
  for (j in c("A3S", "A5S", "intron"))
  ...
  for (r in 1:nrow(df)){
    if (r > 0){
  df$chromosome[r] <- result[[r]][1]
  df$start[r] <- result[[r]][3]
  df$end[r] <- result[[r]][length(result[[1]])]
  df$strand[r] <- result[[r]][2]
    }
  }
  df <- df[,-c(2:4)]
  colnames(df) <- c("Unique Peak ID", "chromosome", "starting position", "ending position", "Strand")
  write.table(df, file = paste0(i, "_", j, "_HOMER.tsv"), sep = "\t", quote = F, row.names = F)
}
```

# Chunk specifically for cassette exon events:
```{r}
setwd("E:/Zach Wolfe's JUM analysis/JUM_results")

  # Function to create new vectors
  create_vectors <- function(vec) {
  # Create two new vectors by extracting required values from input vector
  new_vec1 <- c(vec[1], vec[2], vec[3], vec[4])
  new_vec2 <- c(vec[1], vec[2], vec[5], vec[6])
  
  list(new_vec1, new_vec2)
}


for (i in singletypes){
  df <- deltaPSI_sums_list[[paste0("cassette_event_ID_", i, "_count.csv")]]
  df <- df[df$`sum of all first deltaPSIs for AS_event_ID` > 5,]   ## change sum of all first deltaPSIs threshold here  
  df <- rbind(df, df)
  df <- df[order(df$cassette_event_ID),]
  result <- lapply(df$cassette_event_ID, function(x) unlist(strsplit(x, split = "[_]")))
  result <- lapply(result, function(x) x[x != ""])

  # Apply the create_vectors function to each element of the list
  new_result <- lapply(result, create_vectors)
  new_result <- new_result[-seq(2, length(new_result), by = 2)]
  new_result <- unlist(new_result, recursive = FALSE)
  ...
  for (r in 1:nrow(df)){
    if (r > 0){
  df$chromosome[r] <- new_result[[r]][1]
  df$start[r] <- new_result[[r]][3]
  df$end[r] <- new_result[[r]][length(new_result[[1]])]
  df$strand[r] <- new_result[[r]][2]
    }
  }
  df <- df[,-c(2:4)]
  colnames(df) <- c("Unique Peak ID", "chromosome", "starting position", "ending position", "Strand")
  write.table(df, file = paste0(i, "_cassette_HOMER.tsv"), sep = "\t", quote = F, row.names = F)
}
```


# Create .bed file which will be used for MEME:
```{r}
setwd("E:/Zach Wolfe's JUM analysis/JUM_results")
