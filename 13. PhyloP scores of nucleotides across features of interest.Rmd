...
# Once you have run the Unix script average_phylop_score_for_cassette_exons.txt on HPCC, import your average scores for each IR event here:
```{r}
cassette_phylop <- read_tsv("phyloP_scores_combined.txt")
cassette_phylop <- as.data.frame(cassette_phylop)
...
# In the last regression, which data points represent cassette exons with high abs. uniqueness values AND high conservation scores?
```{r}
# Define thresholds:
...
highlight_plot <- ggplot(combined_data, aes(x = Highest_Abs, y = Cassette_Exon_Score)) +
  ...
  labs(
    ...
    x = "Absolute Highest Uniqueness Value",
    y = "Cassette Exon PhyloP Score") +
  theme_minimal()
