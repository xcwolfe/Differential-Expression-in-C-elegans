...
# Why doesn't the following chunk work?
# It's because you can't do rep x rep DESeq. They disabled this ability 2 versions ago. It has to be a broader type x type comparison. Otherwise, we might as well just make normal graphs and normal heatmaps with no DESeq
```{r}
normalized_counts_results <- matrix(nrow = 160, ncol = 160, dimnames = list(singlereps,singlereps))
normalized_counts_results[is.na(new_results)] <- 0
...
normalized_counts_results[i,j] <- print(length(which(abs(res$log2FoldChange)>2 & res$padj < .05))) # Change values if desired
    }
    if(i == j) next
...
write.csv(normalized_counts_results, file = "normalized_counts_results")
```


# Here we will make another matrix differentiating the number of significant genes between each treatment type (combined with above chunk on 7-28):
```{r}
