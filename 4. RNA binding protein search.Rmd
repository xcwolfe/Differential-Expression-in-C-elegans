...
# Save as .csv:
```{r}
write.csv(topDEgene[1641:6560,], file = "topDifferentiallyExpressedRBPsbyComparison.csv")
...
log2foldchange_per_RBP <-  matrix(nrow = 41, ncol = 484, dimnames = list(singletypes, mined_RBPs$WormBase.Gene.ID))
...
```{r}
write.csv(log2foldchange_per_gene, file = "sum of log2foldchange per RBP.csv")
```

# Backup read.csv:
```{r}
log2foldchange_per_gene <- read.csv("sum of log2foldchange per RBP.csv", row.names = 1)
```
...
```{r}
logSums_RBP_table <- matrix(nrow = 484, ncol = 41, dimnames = list(RBP_list,singletypes))
...
```{r}
maxs <- rowMaxs(as.matrix(logSums_RBP_table[,1:41]))
logSums_RBP_table <- cbind(logSums_RBP_table, maxs)
logSums_RBP_table <- logSums_RBP_table[order(unlist(logSums_RBP_table[,42]), decreasing = TRUE),]
logSums_RBP_table <- logSums_RBP_table[,-42]

# Now may be a good time to change gene names from WBGeneXXXXXXXX to common gene names using WormBase's simple mine tool:
RBP_common_names <- read.csv("RBP_common_names.csv") # see "RBP_common_names.csv" for a list of the 265 RBPs used in this portion of the analysis
